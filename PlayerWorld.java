 ...
 * Theacher:Mr.Hardman
 * Lab#1, program #0
 * Date Lest Modified: February 22nd, 2018
 */
public class PlayerWorld extends World
{
    ...
    /**
     * PlayerWorld is the Constructor for objects of class PlayWorld.
     * 
     * @patam There are no parameters
    ...
    /**
     * drawLines draws the 10*10 grid that we can place our ships inside for 
     * the game Battleship
     * 
     * @parm There are no Parameters
    ...
    /**
     * displayTurn shows various messges for the different stages of the game
     * 
     * @param There are no parameters
     * @return Nothing is returned
     */
    private void displayTurn()
    {
        if( firstTurn == true )
        {
            ...
            {
                JOptionPane.showMessageDialog( null,"Player one, please select where your ships should be placed", "Please Please Ships", JOptionPane.PLAIN_MESSAGE );
    ...
    /**
     * fillBoard fills the numeric represention of the gameboard with 1's
     * 
     * @param There are no parameters
     * @return Nothing is returned
     */
            ...
            for( int j = 0; j < (toAdd.getSize()+1) / 2; j++ )
            {
                ...
                else
                {
                    if( rotation == 0 || rotation == 180 )
                    {
                        board[ (currentY - 36) / 72 ][ (currentX - 36) / 72 - j ] = 1;
                        board[ (currentY - 36) / 72 ][ (currentX - 36) / 72 + j ] = 1;
                        board[ (currentY - 36) / 72 ][ (currentX - 36) / 72 + j + i ] = 1;
                    }
                    else
                    {
                        board[ (currentY - 36) / 72 - j ][ (currentX - 36) / 72 ] = 1;
                        board[ (currentY - 36) / 72 + j ][ (currentX - 36) / 72 ] = 1;
                        board[ (currentY - 36) / 72 + j + i ][ (currentX - 36) / 72 ] = 1;
                    }
                }
            }
        }
    ...
    /**
     * displayBoard displays the numerical representation of the gameboard 
     * where l's indicate that a ship was placed at that location and 0's
     * indicate that no ship is at that location
     * 
     * @param There are no paraeters
     * @return Nothing is returned
     */
